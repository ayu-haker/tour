...
import {
  ...
  Restaurant,
  MenuItem,
} from "@/components/food/RestaurantCard";
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "eta":
        ...
        break;
      case "cost_low":
        list = [...list].sort((a, b) => a.costForTwo - b.costForTwo);
        break;
      case "cost_high":
        list = [...list].sort((a, b) => b.costForTwo - a.costForTwo);
        break;
    }
    return list;
  ...
  function addToCart(item: MenuItem, r: Restaurant) {
    setCart((prev) => {
      const idx = prev.findIndex(
        (ci) => ci.id === item.id && ci.restId === r.id,
      );
      if (idx >= 0) {
        const copy = [...prev];
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                id="veg"
                checked={vegOnly}
                onCheckedChange={(v) => setVegOnly(Boolean(v))}
              />
          ...
          <div className="flex flex-wrap gap-2">
            {CUISINES.map((c) => (
              <button
                ...
                className={`text-xs px-3 py-1 rounded-full border ${cuisines.includes(c) ? "bg-primary text-primary-foreground" : "bg-card"}`}
              >
                {c}
              </button>
            ))}
            ...
            <CardContent className="space-y-3">
              <div className="flex gap-2">
                ...
                <Badge variant="outline">
                  {userLoc ? "Location set" : "Click map to set"}
                </Badge>
              </div>
              <LeafletMap
                markers={markers}
                ...
                path={path}
                center={
                  (userLoc || filtered[0]?.position || [28.61, 77.2]) as [
                    number,
                    number,
                  ]
                }
