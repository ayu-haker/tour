...
const BASE_HOTELS: Hotel[] = [
  {
    ...
    stars: 5,
    price: 8500,
    position: [28.6139, 77.213],
  },
  {
    id: "dl2",
    name: "Connaught Suites",
    city: "Delhi",
    stars: 4,
    price: 5200,
    position: [28.6289, 77.218],
  },
  {
    id: "mum1",
    name: "Marine View",
  ...
  },
  {
    ...
    stars: 4,
    price: 6500,
    position: [19.0607, 72.836],
  },
  {
    id: "blr1",
    name: "Cubbon Park Hotel",
    city: "Bengaluru",
    stars: 4,
    price: 4800,
    position: [12.976, 77.592],
  },
  {
    id: "blr2",
  ...
  },
  {
    ...
    stars: 4,
    price: 5200,
    position: [9.964, 76.242],
  },
];

const USER_PLACES_KEY = "tour.userPlaces";

const STATE_HOTELS: Hotel[] = Object.entries(STATE_CITIES).flatMap(
  ([state, cities]) =>
    cities.map((c, idx) => ({
      id: `st-${state}-${idx}`,
...
function hotelsFromPlaces(): Hotel[] {
  ...
  for (const p of places) {
    ...
    list.push({
      id: `pl-${p.id}-1`,
      name: `${p.title || "Tourist Spot"} Residency`,
      city: p.title || "Tourist Spot",
      stars: 4,
      price: 4200,
      position: [lat + 0.01, lng + 0.01] as [number, number],
    });
    list.push({
      id: `pl-${p.id}-2`,
      name: `${p.title || "Tourist Spot"} View`,
      city: p.title || "Tourist Spot",
      stars: 5,
      price: 7800,
      position: [lat - 0.008, lng - 0.008] as [number, number],
    });
  }
  return list;
}
...
export default function Hotels() {
  ...
  const allHotels = useMemo(
    () => [...BASE_HOTELS, ...STATE_HOTELS, ...hotelsFromPlaces()],
    [],
  );
  ...
  const markers: MapMarker[] = results.map((h) => ({
    id: h.id,
    position: h.position,
    title: `${h.name} • ₹${h.price.toLocaleString("en-IN")}`,
  }));
  ...
  return (
    <SiteLayout>
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Hotel Booking (India)</CardTitle>
          </CardHeader>
          ...
          <Card key={h.id} className="hover:shadow-md transition-shadow">
            ...
            <CardContent>
              ...
              <div className="mt-2 font-semibold">
                ₹{h.price.toLocaleString("en-IN")}/night
              </div>
              <Button className="mt-3" onClick={() => book(h)}>
                Book Now
              </Button>
            </CardContent>
          </Card>
