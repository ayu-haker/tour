...
export default async function handler(req: Request) {
  try {
    ...
    if (!q) return new Response(JSON.stringify({ result: null }), { status: 200, headers: { "Content-Type": "application/json" } });
    const sr = await fetch(wikiSearchUrl(q));
    const sdata = await sr.json().catch(() => ({} as any));
    const page = sdata?.pages?.[0];
    if (!page) return new Response(JSON.stringify({ result: null }), { status: 200, headers: { "Content-Type": "application/json" } });
