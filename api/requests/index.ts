...
export default async function handler(req: Request) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!body.type || !body.payload) return new Response(JSON.stringify({ error: 'type and payload are required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    ...
    return new Response(JSON.stringify({ error: 'Method not allowed' }), { status: 405, headers: { 'Allow': 'GET, POST', 'Content-Type': 'application/json' } });
  } catch (e: any) {
